  ...
  signInWithEmailAndPassword(auth, email.value, password.value)
    .then((userCredential) => {
      ...
      if (!user.emailVerified) {
        ...
        window.location.href = "dashboard_per.html";
      }
    })
    .catch((error) => {
      $(this).find('i').css('display','none');
...
$("#refButton").click(function () {
  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then((userCredential) => {
      ...
      if (!user.emailVerified) {
         window.location.herf = "check_inbox_per.html";
        sendEmailVerification(auth.currentUser).then(() => {
          storeData();
        });
      }
...
$("#logout").click(function () {
  signOut(auth)
    .then(() => {
      // Sign-out successful.
      window.location.href = "login_per.html";
...
async function displayData() {
  ...
  if (docSnap.exists()) {
    ...
    document.getElementById("emailfront").innerHTML =
      "ایمیل:" + emailfront.email;
    document.getElementById("coins").innerHTML =
      "ل سکه های جمع آوری شده: " + emailfront.coins;
...
else if(pathname=='/account_per.html'){
  displayData();
}

if(pathname == '/earn-more_per.html')
{ 
  displayData();
